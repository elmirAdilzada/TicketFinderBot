...
def _check_tracked_dates_for_route(client: ADYApiClient, route: dict, route_snapshot: DateSnapshot) -> None:
    """
    Check tracked dates for all users and notify on seat changes for a specific route.
    Only fetches traintrip details for dates that are already in the snapshot
    ...
    if route.get("notify_only_on_empty") or not route_snapshot:
        return
    ...
    for chat_id, user_tracks in tracked.items():
        for track in user_tracks:
            ...
            for date_val in sorted(route_snapshot.dates.keys()):
                ...
                try:
                    ...
                    # Bubble up – the main loop will handle this natively
                    if updated:
    ...
    if updated:
        set_setting("TRACKED_DATES", tracked)
        log.info("Tracked dates checked for %s and updated (%d API calls).", label, api_calls)
    else:
        log.debug("Tracked dates checked for %s, no changes (%d API calls).", label, api_calls)
    ...
    while True:
        ...
        for route in ROUTES:
            ...
            try:
                ...
                if new_snapshot:
                    _check_tracked_dates_for_route(client, route, new_snapshot)
            except CloudflareChallenge:
